...
1. 下载 `SomaDynamics_1.0.1.0-with-MMD-Stabilizer-v1.2.1.zip`。
...
不使用 MMD 时，只安装 `SomaDynamics_1.0.1.0.zip` 即可。只需要修复 MMD 头发或裙摆时，
...
- 从 1.0.1.0 起，Studio 换角色、换服装、切换或重播动作时会自动执行同类清理，并在
  新姿态写入后延迟两帧再次重置，避免沿用上一个角色或动作的形变。
